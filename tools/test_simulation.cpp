...
public:
  ...
  static void runBulkSession(int numHands, const std::string &csvFilename) {
    ...
    csv << "HandID,Round,Board,Bot1_Hole,Bot1_Equity,Bot1_Act,Bot1_Sizing,Bot2_"
           "Hole,Bot2_Equity,Bot2_Act,Bot2_Sizing,Bot2_EstProfile\n";
    ...
    for (int i = 0; i < numHands; ++i) {
      ...
      AI::RangeParser::parseHoldemRange(
          r2, "22+, A2s+, K2s+, Q2s+, J2s+, T2s+, 92s+, 82s+, A2o+, K2o+, "
              "Q2o+, J2o+, T2o+");
      ...
      for (int r = 0; r < 4; ++r) {
        ...
        PlayerAction act1 =
            AI::InferenceEngine::decideAction(state, 1, *bot1, bot1View);
        ...
        PlayerAction act2 =
            AI::InferenceEngine::decideAction(state, 2, *bot2, bot2View);
        ...
        for (const auto &p : state.getPlayers()) {
          if (p.id == 1)
            p1HoleStr = handToString(p.faceDownCards);
          if (p.id == 2)
            p2HoleStr = handToString(p.faceDownCards);
        }

        csv << i << "," << r << "," << boardStr << "," << p1HoleStr << ","
            << std::fixed << std::setprecision(3) << bot1Equity << ","
            << actionToString(act1.type) << "," << act1.absoluteAmount << ","
            << p2HoleStr << "," << std::fixed << std::setprecision(3)
            << bot2Equity << "," << actionToString(act2.type) << ","
            << act2.absoluteAmount << "," << b2Prof << "\n";
      ...
      if (i == numHands / 2) {
        std::cout << "[Event] Mid-session Hot Swap! Bot 2 replaced with a Nit "
                     "profile.\n";
