...
public:
    /**
     * @brief Runs a simulation loop for a given number of hands and stores
     * results in CSV.
     * @param numHands The number of hands to simulate.
     * @param csvFilename Output file for tracking game states and Bayesian
     * features.
     */
    static void runBulkSession(int numHands, const std::string& csvFilename) {
        std::ofstream csv(csvFilename);
        csv << "HandID,Round,Board,Bot1_Hole,Bot1_Equity,Bot1_Act,Bot1_Sizing,"
               "Bot2_Hole,Bot2_Equity,Bot2_Act,Bot2_Sizing,Bot2_EstProfile\n";
        ...
        auto bot1 = AI::BotFactory::createBot(AI::ActionProfile::Balanced,
                                              AI::SizingProfile::Fixed, 42);
        auto bot2 = AI::BotFactory::createBot(AI::ActionProfile::LAG,
                                              AI::SizingProfile::Gaussian, 43);
        ...
        LOG_INFO("Starting Simulation for " + std::to_string(numHands) +
                 " games...");
        ...
        for (int i = 0; i < numHands; ++i) {
            ...
            AI::RangeParser::parseHoldemRange(
                r1, "22+, A2s+, KTs+, QTs+, JTs, ATo+, KQo");
            AI::RangeParser::parseHoldemRange(
                r2,
                "22+, A2s+, K2s+, Q2s+, J2s+, T2s+, 92s+, 82s+, A2o+, K2o+, "
                "Q2o+, J2o+, T2o+");
            ...
            for (int r = 0; r < 4; ++r) {
                std::vector<double> eq1 =
                    Engine::Analyzer::calculateEquity(state, bot1View, 1000);
                double bot1Equity = eq1[0];  // Assuming player 1 is index 0

                // Perform Inference for bot 1
                PlayerAction act1 = AI::InferenceEngine::decideAction(
                    state, 1, *bot1, bot1View, &tracker);
                state.recordAction(1, act1.type, act1.absoluteAmount);

                int potPct1 = state.getPot() > 0
                                  ? (act1.absoluteAmount * 100) / state.getPot()
                                  : 0;
                tracker.updateFromAction(1, act1.type, potPct1);

                std::vector<double> eq2 =
                    Engine::Analyzer::calculateEquity(state, bot2View, 1000);
                double bot2Equity = eq2[1];  // Assuming player 2 is index 1

                // Perform Inference for bot 2
                PlayerAction act2 = AI::InferenceEngine::decideAction(
                    state, 2, *bot2, bot2View, &tracker);
                state.recordAction(2, act2.type, act2.absoluteAmount);

                int potPct2 = state.getPot() > 0
                                  ? (act2.absoluteAmount * 100) / state.getPot()
                                  : 0;
                ...
                csv << i << "," << r << "," << boardStr << "," << p1HoleStr
                    << "," << std::fixed << std::setprecision(3) << bot1Equity
                    << "," << actionToString(act1.type) << ","
                    << act1.absoluteAmount << "," << p2HoleStr << ","
                    << std::fixed << std::setprecision(3) << bot2Equity << ","
                    << actionToString(act2.type) << "," << act2.absoluteAmount
                    << "," << b2Prof << "\n";

                if (act1.type == ActionType::Fold ||
                    act2.type == ActionType::Fold) {
            ...
            if (i == numHands / 2) {
                LOG_INFO(
                    "[Event] Mid-session Hot Swap! Bot 2 replaced with a Nit "
                    "profile.");
                bot2 = AI::BotFactory::createBot(AI::ActionProfile::Nit,
                                                 AI::SizingProfile::Fixed, 44);
        ...
        LOG_INFO("Simulation completed. Data saved to " + csvFilename);
    }
};
