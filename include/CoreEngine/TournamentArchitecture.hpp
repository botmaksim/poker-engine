...
#include "../BotProfiles/BotFactory.hpp"
...
namespace PokerEngine {
    namespace Tournament {
        ...
        // -----------------------------------------------------------------------------
        // Data structures for logging (optimized for cache lines and
        // serialization)
        // -----------------------------------------------------------------------------
...
#pragma pack(push, 1)
        ...
        struct ActionLogRecord {
            ...
            uint8_t position;  ///< Player position at the table (0: BTN, 1: SB,
                               ///< 2: BB, ...)
            uint32_t stackBefore;  ///< Player stack before taking the action
            uint8_t
                actionType;  ///< Action type (Fold, Check, Call, Bet, Raise)
        ...
        struct PlayerMetricRecord {
            ...
            float objectiveEquity;  ///< True (mathematical) equity with known
                                    ///< cards
            float evLoss;  ///< Mistake Analysis: EV loss relative to perfect
                           ///< action
            ...
            float tiltIndex;  ///< Reaction to variance (aggression deviation
                              ///< after loss)
        };
...
#pragma pack(pop)
        ...
        struct PlayerSessionStats {
            ...
            float getVPIP() const {
                return handsPlayed > 0
                           ? (float)voluntarilyPutInPot / handsPlayed
                           : 0.0f;
            }
            float getPFR() const {
                return handsPlayed > 0 ? (float)preflopRaises / handsPlayed
                                       : 0.0f;
            }
            float getAF() const {
                return passiveActions > 0
                           ? (float)aggressiveActions / passiveActions
                           : (float)aggressiveActions;
            }
            float getTiltIndex() const {
                return consecutiveLosses >= 3 ? getAF() * 1.5f : getAF();
            }  // Approximate tilt index
        };
        ...
        /**
         * @brief Class for high-speed background tracking, does not block the
        ...
        public:
            ...
            /**
             * @brief Destructor. Flushes data and cleanly terminates the
             * thread.
             */
        ...
        private:
            ...
            // Fast lock-free queue (in a real project, consider
            // moodycamel::ConcurrentQueue) Simplified double-buffering
        ...
        public:
             ...
             */
            void runHeadsUpMatch(
                int bot1ProfileID, int bot2ProfileID, int numHands,
                GameVariant variant = GameVariant::TexasHoldem);
             ...
             * @param variant Poker variant
             */
            void runMassSimulation(
                int maxHands, const std::vector<int>& botPool,
                GameVariant variant = GameVariant::TexasHoldem);

        private:
            void executeHand(Engine::GameState& state,
                             std::vector<int>& activeBotProfiles,
                             uint32_t handID);
            void computeMetricsAndLog(Engine::GameState& state,
                                      const std::vector<int>& activeBotProfiles,
                                      uint32_t handID);
        ...
        /**
         * @brief Overall tournament coordinator, controls threads and
         * simulation phases.
         */
        ...
        public:
             ...
             * @param numThreads Number of worker threads (defaults to hardware
             * cores)
             */
            ...
            /**
             * @brief Executes Phase 1 (Round-Robin matches across bot
             * combinations).
            ...
            /**
             * @brief Executes Phase 2 (Mass sessions tracking bot pools at
             * random).
             * @param botPool Discovered/available pool of tournament bots
             * @param totalMassSessions Number of independent mass sessions to
             * execute
             * @param variant Poker variant
             */
            void executePhase2(const std::vector<int>& botPool,
                               uint64_t totalMassSessions = 1000,
                               GameVariant variant = GameVariant::TexasHoldem);
