...
namespace PokerEngine {
    namespace Engine {
        ...
        GameState::GameState(const TableConfig& tableConfig,
        ...
        void GameState::advanceRound() {
            ...
            if (currentBettingRound <= rules->getMaxBettingRounds()) {
                ...
                rules->dealStreet(players, boardCards, deck,
                                  currentBettingRound);
                for (auto& p : players) {
        ...
        void GameState::recordAction(int playerId, ActionType type,
            ...
            if (it != players.end()) {
                ...
                } else if (type == ActionType::PostBlind ||
                           type == ActionType::BringIn ||
                           type == ActionType::Ante) {
                    if (absoluteAmount > currentWager)
                        currentWager = absoluteAmount;
                }
        ...
        bool GameState::isActionLegal(int playerId, ActionType type,
            ...
            if (type == ActionType::Call) {
                // Player must exactly match currentWager, OR go all-in for
                // whatever they have left
                if (relativeAmount == callAmount && p.chipCount >= callAmount)
                    return true;
                if (relativeAmount == p.chipCount && p.chipCount < callAmount)
                    return true;
                return false;
            }
            ...
            if (type == ActionType::Bet || type == ActionType::Raise) {
                ...
                if (config.limitType == LimitType::FixedLimit) {
                    if (config.maxRaisesPerRound > 0 &&
                        raisesThisRound >= config.maxRaisesPerRound &&
                        relativeAmount < p.chipCount)
                        return false;
                    int limitBet = (currentBettingRound < 2) ? config.smallBlind
                                                             : config.bigBlind;
                    if (config.smallBlind == 0)
                        limitBet = config.bigBlind;  // fallback
                    return raiseAmount == limitBet ||
                           relativeAmount == p.chipCount;
                }
            ...
            return true;  // PostBlind, Ante, BringIn allowed directly via
                          // engine loops
        }
        ...
        const std::vector<std::vector<PlayerAction>>&
        GameState::getBettingHistory() const {
            return bettingHistory;
        }
        const Interfaces::IGameRules* GameState::getRules() const {
            return rules.get();
        }
