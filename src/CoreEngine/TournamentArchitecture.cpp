...
#include "../../include/BotProfiles/BotFactory.hpp"
...
#include "../../include/CoreEngine/Variants/SevenCardStud.hpp"
#include "../../include/CoreEngine/Variants/ShortDeck.hpp"
...
namespace PokerEngine {
    namespace Tournament {
        ...
        std::unique_ptr<Interfaces::IGameRules> createGameVariant(
            GameVariant variant) {
            switch (variant) {
                case GameVariant::Omaha:
                    return std::make_unique<Games::Omaha>();
                case GameVariant::ShortDeck:
                    return std::make_unique<Games::ShortDeck>();
                case GameVariant::SevenCardStud:
                    return std::make_unique<Games::SevenCardStud>();
                case GameVariant::Razz:
                    return std::make_unique<Games::Razz>();
                case GameVariant::FiveCardDraw:
                    return std::make_unique<Games::FiveCardDraw>();
                case GameVariant::DeuceToSevenTripleDraw:
                    return std::make_unique<Games::DeuceToSevenTripleDraw>();
                case GameVariant::TexasHoldem:
                default:
                    return std::make_unique<Games::TexasHoldem>();
            }
        }
        ...
        AsyncTournamentLogger::AsyncTournamentLogger(
            ...
            backgroundThread =
                std::thread(&AsyncTournamentLogger::ioWorkerLoop, this);
        }
        ...
        void AsyncTournamentLogger::logMetric(
            const PlayerMetricRecord& record) {
            std::lock_guard<std::mutex> lock(mtx);
            if (usingBufferA) {
        ...
        void AsyncTournamentLogger::ioWorkerLoop() {
            std::ofstream actionFile(outputPath + "_actions.csv",
                                     std::ios::app);
            std::ofstream metricFile(outputPath + "_metrics.csv",
                                     std::ios::app);
            ...
            actionFile << "SessionID,HandID,PlayerID,BotProfileID,Street,"
                          "Position,StackBefore,ActionType,ActionAmount,SPR\n";
            metricFile << "SessionID,HandID,PlayerID,HoleCardsMask,"
                          "BotEstEquity,ObjEquity,EVLoss,Exploitability,VPIP,"
                          "PFR,AggrFactor,TiltIndex\n";
            ...
            while (isRunning) {
...

                {
                    ...
                    cv.wait_for(lock, std::chrono::milliseconds(1000), [this] {
                        return !isRunning ||
                               (!usingBufferA && !actionBufferB.empty()) ||
                               (usingBufferA && !actionBufferA.empty());
                    });
                ...
                for (const auto& a : localActions) {
                    actionFile << a.sessionID << "," << a.handID << ","
                               << a.playerID << "," << a.botProfileID << ","
                               << (int)a.street << "," << (int)a.position << ","
                               << a.stackBefore << "," << (int)a.actionType
                               << "," << a.actionAmount << "," << a.spr << "\n";
                }

                for (const auto& m : localMetrics) {
                    metricFile
                        << m.sessionID << "," << m.handID << "," << m.playerID
                        << "," << m.holeCardsMask << "," << m.botEstimatedEquity
                        << "," << m.objectiveEquity << "," << m.evLoss << ","
                        << m.exploitability << "," << m.vpip << "," << m.pfr
                        << "," << m.aggressionFactor << "," << m.tiltIndex
                        << "\n";
                }
            }
        }
        ...
        TournamentSession::TournamentSession(uint64_t id,
                                             AsyncTournamentLogger& logger)
            : sessionID(id), logger(logger) {}

        void TournamentSession::runHeadsUpMatch(int bot1ProfileID,
                                                int bot2ProfileID, int numHands,
                                                GameVariant variant) {
        ...
        void TournamentSession::runMassSimulation(
            int maxHands, const std::vector<int>& botPool,
            GameVariant variant) {
            auto game = createGameVariant(variant);
            TableConfig config{LimitType::NoLimit, 5, 10, 0, 0, 0};
            Engine::GameState state(config, std::move(game));
            ...
            for (int i = 0; i < maxHands; ++i) {
                ...
                if (i > 0 && i % 15 == 0) {
                    int swapIdx = rand() % 5;
                    activeBotProfiles[swapIdx] =
                        botPool[rand() % botPool.size()];
                }
        ...
        void TournamentSession::executeHand(Engine::GameState& state,
                                            std::vector<int>& activeBotProfiles,
                                            uint32_t handID) {
            ...
            thread_local static const AI::Range defaultHoldemRange = []() {
                AI::Range r;
                AI::RangeParser::parseHoldemRange(
                    r,
                    "22+, A2s+, K2s+, Q2s+, J2s+, T2s+, 92s+, 82s+, A2o+, "
                    "K2o+, Q2o+, J2o+, T2o+");
                return r;
            }();
            ...
            for (const auto& player : state.getPlayers()) {
                oppRanges[player.id] = defaultHoldemRange;
            }

            while (state.getCurrentBettingRound() <=
                   state.getRules()->getMaxBettingRounds()) {
                int active = 0;
                for (auto& p : state.getPlayers())
                    if (!p.hasFolded) active++;
                if (active <= 1) break;
                ...
                for (size_t i = 0; i < state.getPlayers().size(); ++i) {
                    ...
                    AI::ActionProfile aProf =
                        static_cast<AI::ActionProfile>(profileId % 5);
                    AI::SizingProfile sProf =
                        static_cast<AI::SizingProfile>((profileId / 5) % 3);
                    auto bot =
                        AI::BotFactory::createBot(aProf, sProf, profileId);
                    ...
                    if (street == 0 && act.type == ActionType::Raise &&
                        !sessionStats[player.id].hasPfrThisHand) {
                    ...
                    logReq.spr = (float)player.chipCount /
                                 (state.getPot() > 0 ? state.getPot() : 1);
        ...
        void TournamentSession::computeMetricsAndLog(
            Engine::GameState& state, const std::vector<int>& activeBotProfiles,
            uint32_t handID) {
            // 1. Calculate actual objective equity via Analyzer (5 iterations
            // for speed)
            std::map<int, AI::Range> emptyRanges;
            std::vector<double> objEquities =
                Engine::Analyzer::calculateEquity(state, emptyRanges, 5);
            ...
            for (size_t i = 0; i < state.getPlayers().size(); ++i) {
                ...
                float evLoss =
                    (!p.hasFolded)
                        ? 0.0f
                        : (objEq > 0.5f ? (objEq * state.getPot()) : 0.0f);
                ...
                metric.botEstimatedEquity =
                    objEq * 0.98f;  // Proxy for bot's internal tracking
        ...
        TournamentCoordinator::TournamentCoordinator(
            const std::string& outputDir, int numThreads)
            : workerThreadCount(numThreads) {
            globalLogger = std::make_unique<AsyncTournamentLogger>(outputDir);
        }

        void TournamentCoordinator::executePhase1(
            const std::vector<int>& botPool, GameVariant variant) {
            LOG_INFO(std::string("[Phase 1] Round-Robin Started on ") +
                     std::to_string(workerThreadCount) + " threads");
            ...
            for (int t = 0; t < workerThreadCount; ++t) {
                workers.emplace_back([this, &pairs, &pairIdx, &completed,
                                      totalPairs, variant]() {
                    while (true) {
                        ...
                        session.runHeadsUpMatch(
                            pairs[idx].first, pairs[idx].second, 10,
                            variant);  // 10 hands per requirement

                        size_t c = completed.fetch_add(1) + 1;
                        if (c % 10 == 0 || c == totalPairs) {
                            std::lock_guard<std::mutex> lock(consoleMutex);
                            LOG_INFO(std::string("[Phase 1] Progress: ") +
                                     std::to_string(c) + " / " +
        ...
        void TournamentCoordinator::executePhase2(
            const std::vector<int>& botPool, uint64_t totalMassSessions,
            GameVariant variant) {
            LOG_INFO(std::string("[Phase 2] Mass Simulation Started (") +
                     std::to_string(totalMassSessions) + " sessions)");
            std::atomic<uint64_t> sessionCounter{0};
            std::atomic<uint64_t> completedPhase2{0};
            std::vector<std::thread> workers;

            for (int t = 0; t < workerThreadCount; ++t) {
                workers.emplace_back([this, &botPool, &sessionCounter,
                                      &completedPhase2, totalMassSessions,
                                      variant]() {
                    while (true) {
                        ...
                        session.runMassSimulation(
                            15, botPool, variant);  // 15 hands before swap

                        uint64_t c = completedPhase2.fetch_add(1) + 1;
                        if (c % 10 == 0 || c == totalMassSessions) {
                            float pct = (float)c / totalMassSessions * 100.0f;
                            std::string streamStr;
                            streamStr += "[Phase 2] Progress: ";
                            streamStr += std::to_string(c);
                            streamStr += " / ";
                            streamStr += std::to_string(totalMassSessions);
                            streamStr += " mass sessions completed.";
