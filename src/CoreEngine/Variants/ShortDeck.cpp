...
namespace PokerEngine::Games {
    ...
    void ShortDeck::postMandatoryBets(std::vector<Player>& players, int& pot,
    ...
    void ShortDeck::dealStreet(std::vector<Player>& players, Hand& boardCards,
                               Hand& deck, int round) const {
        if (round == 0) {
            auto it = std::remove_if(deck.begin(), deck.end(), [](CardMask c) {
                int rank = Bitboard::getRank(c);
                return rank >= 0 && rank <= 3;  // Remove 2, 3, 4, 5
            });
    ...
    int ShortDeck::evaluateHand(const Hand& holeCards,
                                const Hand& boardCards) const {
        Hand allCards = holeCards;
        allCards.insert(allCards.end(), boardCards.begin(), boardCards.end());
        return PokerEngine::Engine::UniversalEvaluator::evaluateShortDeck(
            allCards);
    }

    int PokerEngine::Games::ShortDeck::evaluateHandFast(
