    ...
    for (int r = rank1; r <= rank2; ++r) {
        for (int s1 = 0; s1 < 4; ++s1) {
            for (int s2 = s1 + 1; s2 < 4; ++s2) {
                Hand h = {Bitboard::makeCard(r, s1), Bitboard::makeCard(r, s2)};
...
static void addSuited(AI::Range& range, int high, int low_min, int low_max) {
    for (int low = low_min; low <= low_max; ++low) {
        if (high == low) continue;
        for (int s = 0; s < 4; ++s) {
            Hand h = {Bitboard::makeCard(high, s), Bitboard::makeCard(low, s)};
            range.addCombo(h, 1.0);
        }
    }
}

static void addOffsuit(AI::Range& range, int high, int low_min, int low_max) {
    for (int low = low_min; low <= low_max; ++low) {
        if (high == low) continue;
        for (int sh = 0; sh < 4; ++sh) {
            for (int sl = 0; sl < 4; ++sl) {
                if (sh != sl) {
                    Hand h = {Bitboard::makeCard(high, sh), Bitboard::makeCard(low, sl)};
                    range.addCombo(h, 1.0);
                }
            }
        }
...
void RangeParser::parseHoldemRange(AI::Range& range, const std::string& rangeString) {
    std::stringstream ss(rangeString);
    std::string tokenStr;
    
    while (std::getline(ss, tokenStr, ',')) {
        // remove spaces
        tokenStr.erase(std::remove_if(tokenStr.begin(), tokenStr.end(), ::isspace), tokenStr.end());
