...
Session::Session(const TableConfig& config, std::unique_ptr<Interfaces::IGameRules> gameRules)
    : state(std::make_unique<GameState>(config, std::move(gameRules))),
      tracker(std::make_shared<AI::BayesianTracker>()) {
}

void Session::seatBot(int playerId, int startingChips, std::unique_ptr<AI::CompositeBot> bot) {
    state->addPlayer(playerId, startingChips);
    activeBots[playerId] = std::move(bot);
    tracker->initializePlayer(playerId);
}

std::unique_ptr<AI::CompositeBot> Session::replacePlayer(int playerId, std::unique_ptr<AI::CompositeBot> newBot) {
...
void Session::playHands(int numberOfHands) {
    for (int i = 0; i < numberOfHands; ++i) {
        ...
        // In a real implementation we'd check if the hand has concluded yet
