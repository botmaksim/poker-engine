...
AsyncTournamentLogger::AsyncTournamentLogger(const std::string &path)
    : outputPath(path) {
...
void AsyncTournamentLogger::ioWorkerLoop() {
  ...
  actionFile << "SessionID,HandID,PlayerID,BotProfileID,Street,Position,"
                "StackBefore,ActionType,ActionAmount,SPR\n";
  metricFile
      << "SessionID,HandID,PlayerID,HoleCardsMask,BotEstEquity,ObjEquity,"
         "EVLoss,Exploitability,VPIP,PFR,AggrFactor,TiltIndex\n";
  ...
  while (isRunning) {
...

    {
      ...
      cv.wait_for(lock, std::chrono::milliseconds(1000), [this] {
        return !isRunning || (!usingBufferA && !actionBufferB.empty()) ||
    ...
    for (const auto &a : localActions) {
      actionFile << a.sessionID << "," << a.handID << "," << a.playerID << ","
                 << a.botProfileID << "," << (int)a.street << ","
                 << (int)a.position << "," << a.stackBefore << ","
                 << (int)a.actionType << "," << a.actionAmount << "," << a.spr
                 << "\n";
    }

    for (const auto &m : localMetrics) {
      metricFile << m.sessionID << "," << m.handID << "," << m.playerID << ","
...
TournamentSession::TournamentSession(uint64_t id, AsyncTournamentLogger &logger)
    : sessionID(id), logger(logger) {}

void TournamentSession::runHeadsUpMatch(int bot1ProfileID, int bot2ProfileID,
                                        int numHands, GameVariant variant) {
...
void TournamentSession::runMassSimulation(int maxHands,
                                          const std::vector<int> &botPool,
                                          GameVariant variant) {
  auto game = createGameVariant(variant);
  TableConfig config{LimitType::NoLimit, 5, 10, 0, 0, 0};
  Engine::GameState state(config, std::move(game));
...
void TournamentSession::executeHand(Engine::GameState &state,
  ...
  thread_local static const AI::Range defaultHoldemRange = []() {
    ...
    AI::RangeParser::parseHoldemRange(
        r, "22+, A2s+, K2s+, Q2s+, J2s+, T2s+, 92s+, 82s+, A2o+, K2o+, Q2o+, "
           "J2o+, T2o+");
    return r;
  }();
  ...
  while (state.getCurrentBettingRound() <=
    ...
    for (size_t i = 0; i < state.getPlayers().size(); ++i) {
      ...
      PlayerAction act =
          AI::InferenceEngine::decideAction(state, player.id, *bot, view);
      state.recordAction(player.id, act.type, act.absoluteAmount);

      uint8_t street = state.getCurrentBettingRound();

      // Tracking metrics
      if (act.type == ActionType::Call || act.type == ActionType::Bet ||
      ...
      logReq.spr =
          (float)player.chipCount / (state.getPot() > 0 ? state.getPot() : 1);
...
void TournamentSession::computeMetricsAndLog(
    ...
    uint32_t handID) {
  // 1. Calculate actual objective equity via Analyzer (5 iterations for speed)
...
TournamentCoordinator::TournamentCoordinator(const std::string &outputDir,
                                             int numThreads)
...
void TournamentCoordinator::executePhase1(const std::vector<int> &botPool,
                                          GameVariant variant) {
  std::cout << "[Phase 1] Round-Robin Started on " << workerThreadCount
            << " threads\n";
  ...
  for (int t = 0; t < workerThreadCount; ++t) {
    workers.emplace_back(
        [this, &pairs, &pairIdx, &completed, totalPairs, variant]() {
          while (true) {
            ...
            session.runHeadsUpMatch(pairs[idx].first, pairs[idx].second, 10,
                                    variant); // 10 hands per requirement

            size_t c = completed.fetch_add(1) + 1;
            if (c % 10 == 0 || c == totalPairs) {
              std::lock_guard<std::mutex> lock(consoleMutex);
              std::cout << "[Phase 1] Progress: " << c << " / " << totalPairs
                        << " match-ups completed.\n";
            }
          }
        });
...
void TournamentCoordinator::executePhase2(const std::vector<int> &botPool,
                                          uint64_t totalMassSessions,
                                          GameVariant variant) {
  std::cout << "[Phase 2] Mass Simulation Started (" << totalMassSessions
            << " sessions)\n";
  ...
  for (int t = 0; t < workerThreadCount; ++t) {
    workers.emplace_back([this, &botPool, &sessionCounter, &completedPhase2,
                          totalMassSessions, variant]() {
      while (true) {
        uint64_t idx = sessionCounter.fetch_add(1);
        if (idx >= totalMassSessions)
          break;
        ...
        if (c % 10 == 0 || c == totalMassSessions) {
          ...
          std::cout << "[Phase 2] Progress: " << c << " / " << totalMassSessions
                    << " mass sessions completed (" << std::fixed
