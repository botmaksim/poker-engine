...
void Razz::postMandatoryBets(std::vector<Player>& players, int& pot, std::vector<PlayerAction>& history, const TableConfig& config) const {
...
void Razz::dealStreet(std::vector<Player>& players, Hand& boardCards, Hand& deck, int round) const {
...
int Razz::evaluateHand(const Hand& holeCards, const Hand& boardCards) const {
        ...
        if (n == 5) {
            ...
            for (int skip1 = 0; skip1 < 6; ++skip1) {
                for (int skip2 = skip1 + 1; skip2 < 7; ++skip2) {
                    int idx = 0;
                    for (int i = 0; i < 7; ++i) {
                        if (i != skip1 && i != skip2) combo[idx++] = allCards[i];
                    }
                    evalCombo(combo);
                }
            }
