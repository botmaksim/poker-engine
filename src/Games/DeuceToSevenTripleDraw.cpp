...
std::string DeuceToSevenTripleDraw::getName() const { return "2-7 Triple Draw"; }
...
void DeuceToSevenTripleDraw::postMandatoryBets(std::vector<Player>& players, int& pot, std::vector<PlayerAction>& history, const TableConfig& config) const {
...
void DeuceToSevenTripleDraw::dealStreet(std::vector<Player>& players, Hand& boardCards, Hand& deck, int round) const {
    if (round == 0) {
        for(int i = 0; i < 5; i++) {
...
int DeuceToSevenTripleDraw::evaluateHand(const Hand& holeCards, const Hand& boardCards) const {
    Hand allCards = holeCards;
    allCards.insert(allCards.end(), boardCards.begin(), boardCards.end());
    return Engine::UniversalEvaluator::evaluate27Low(allCards);
}
}
